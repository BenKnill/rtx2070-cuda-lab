...
LOG="$LOG_DIR/wsl-oceanfft-gallery-animation-$STAMP.txt"
exec > >(tee -a "$LOG") 2>&1

echo "Build animated oceanFFT gallery assets $STAMP"
...
ffmpeg -y -framerate 16 -i "$FRAME_DIR/frame_%03d.ppm" \
  -vf "fps=16,scale=512:-1:flags=lanczos,palettegen" \
  "$TMP/palette.png"

ffmpeg -y -framerate 16 -i "$FRAME_DIR/frame_%03d.ppm" -i "$TMP/palette.png" \
  -lavfi "fps=16,scale=512:-1:flags=lanczos[x];[x][1:v]paletteuse=dither=bayer:bayer_scale=2" \
  "$DST_DIR/oceanFFT_native_run.gif"

ffmpeg -y -framerate 16 -i "$FRAME_DIR/frame_%03d.ppm" \
  -movflags +faststart \
  -pix_fmt yuv420p \
  "$DST_DIR/oceanFFT_native_run.mp4"

echo
echo "== gif frames =="
if command -v magick >/dev/null 2>&1; then
  magick identify -format '%n frames %w x %h\n' "$DST_DIR/oceanFFT_native_run.gif" | head -n 5
...
echo
echo "== mp4 stream =="
ffprobe -v error -select_streams v:0 -show_entries stream=codec_name,nb_frames,duration,width,height -of default=noprint_wrappers=1 "$DST_DIR/oceanFFT_native_run.mp4"
